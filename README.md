...
- `site/mindmaps/`：每个课件/章节的 SVG 思维导图下载文件。
...
- 原始课件未被修改。
- 当前已纳入 15 个课件文件。
- 当前数据库包含 869 个可检索片段、95 个索引词条。
- 标准题库保留 567 道题：285 道单选、144 道填空、138 道简答。
- 基础题库新增 2090 道低难度题，用于先熟悉课程脉络。
- 网站包含：搜索、教材目录、思维导图、基础练习、标准组卷、综合资料。
- 思维导图以网页文字版展示，并提供 SVG 图片下载链接；过大的图会按章节拆分。
- 构建脚本支持未来 `.pptx` 课件文本提取，并用文件哈希缓存跳过未变化课件。
