...
- `database/`：从课件抽取出的搜索数据库、教材目录、词条索引、基础题库、标准题库。
- `site/`：可直接打开或由 GitHub Pages 发布的静态网站。
- `site/guides/`：按课件生成的 Markdown 考试复习讲义，以及总目录。
- `site/topic_guides/`：按主题重写的高质量通俗讲义，包括 MLP、CNN、RNN、Transformer、图学习、NLP、LLM、总复习资料，以及课堂练习册/答案册。
...
之后新增课件时，把新的 `.ipynb`、`.pptx` 或课程来源 `.md` 放到上一级课件目录，然后运行：
...
- 本工程没有保留零散转换 `.py` 文件。
- 原始课件未被修改。
- 当前已纳入 19 个来源文件，包括新增的图神经网络最后部分 PPT 和课程导学 Markdown。
- 当前数据库包含 1106 个可检索片段、113 个索引词条。
- 标准题库包含 318 道题，只保留单选题、简答题、资料题。
- 基础题库包含 166 道低难度题，只保留单选题、简答题、资料题。
- 网站包含：搜索、教材目录、基础练习、标准组卷、主题书库。
...
- 主题书库额外提供 `深度学习期末复习-总复习资料.md` 下载入口；根目录的 `深度学习期末复习_仅基于原始课件版.md` 会在构建时覆盖更新。
...
- 网站已删除旧图谱页面及相关生成产物。
- 构建脚本支持 `.pptx` 文本提取；图片型 PPT 会逐页调用 macOS Vision OCR，并用文件哈希缓存跳过未变化课件。
