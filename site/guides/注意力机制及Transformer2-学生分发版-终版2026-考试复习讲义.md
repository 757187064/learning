...
**BatchNorm** 的核心含义：在小批量上标准化中间激活，再用可学习参数恢复表达能力，帮助训练更稳定。

它在课件中的考查位置：常考训练/推理阶段统计量不同，以及放在卷积/全连接和激活附近。

如果按课件顺序串起来，可以这样看：**前面的概念 -> BatchNorm -> Attention**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**BatchNorm：标准化激活，稳定训练，加快收敛。**
...
**Batch** 的核心含义：一次送入模型并一起计算损失的一组样本。它决定每次参数更新看到多少样本，也影响显存占用和训练稳定性。

它在课件中的考查位置：常考 batch size、mini-batch、epoch、iteration 的区别，也会结合张量形状中的 N 维度判断。

如果按课件顺序串起来，可以这样看：**Encoder-Decoder -> Batch -> 归一化**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**Batch 是一批样本；Epoch 是全训练集完整训练一遍。**
...
**Dropout** 的核心含义：训练时随机丢弃一部分神经元，迫使网络不要过度依赖某些特征。

它在课件中的考查位置：常考训练阶段启用、推理阶段关闭，以及它缓解过拟合。

如果按课件顺序串起来，可以这样看：**归一化 -> Dropout -> Query**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**Dropout：训练随机失活，推理正常使用。**
...
2. **资料题：** 材料中出现 Dropout，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** 训练时随机丢弃一部分神经元，迫使网络不要过度依赖某些特征。 答题时还要结合课件语境：常考训练阶段启用、推理阶段关闭，以及它缓解过拟合。
...
**Multi-Head Attention** 的核心含义：把注意力分成多个头并行学习不同关系，再拼接融合。

它在课件中的考查位置：常考多头为什么能捕捉不同子空间信息。

如果按课件顺序串起来，可以这样看：**Attention -> Multi-Head Attention -> 后面的概念**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**多头注意力让模型从多个角度看关系。**
...
2. **资料题：** 材料中出现 归一化，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** 把数值缩放到较统一范围，例如图像像素从 0-255 缩放到 0-1。 答题时还要结合课件语境：常考图像预处理为什么要缩放像素值。
...
2. **资料题：** 材料中出现 Query，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** Query 表示当前位置主动提出的查询，用来和 Key 计算匹配度。 答题时还要结合课件语境：常考 Q 与 K 点积得到注意力分数。
...
2. **资料题：** 材料中出现 Self-Attention，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** Q、K、V 来自同一个序列，让序列内部不同位置相互建立联系。 答题时还要结合课件语境：常考它和 Cross-Attention 的区别。
...
**激活函数** 的核心含义：给线性变换加入非线性，使网络能拟合非线性关系。

它在课件中的考查位置：常考没有激活函数时多层线性网络仍等价于线性模型。

如果按课件顺序串起来，可以这样看：**Batch -> 激活函数 -> 归一化**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**仿射变换 + 激活函数 = 神经网络层的基本模式。**
...
**Subsequent Mask** 的核心含义：一种上三角未来位置遮罩，让解码器只能看当前位置及之前。

它在课件中的考查位置：常考 Decoder 自注意力中的未来信息屏蔽。

如果按课件顺序串起来，可以这样看：**Key -> Subsequent Mask -> 位置编码**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**Subsequent Mask 用来挡住未来 token。**
...
2. **资料题：** 材料中出现 激活函数，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** 给线性变换加入非线性，使网络能拟合非线性关系。 答题时还要结合课件语境：常考没有激活函数时多层线性网络仍等价于线性模型。
...
**Epoch** 的核心含义：训练集被模型完整看过一遍，叫一个 epoch。一个 epoch 内通常包含很多个 batch 更新。

它在课件中的考查位置：常考 Epoch、Batch、Iteration 的区别，或判断训练轮数增加对欠拟合/过拟合的影响。

如果按课件顺序串起来，可以这样看：**学习率 -> Epoch -> 后面的概念**。
也就是说，你不是在背一个词，而是在看知识点之间怎样往下推进。

考试作答时可以写成：**1 个 Epoch = 全部训练样本被用过一遍。**
...
2. **资料题：** 材料中出现 学习率，请判断它在本节属于数据、模型结构、训练优化还是输出目标，并说明理由。

   **参考答案：** 控制每次参数沿梯度方向更新的步子大小。学习率太大容易震荡甚至发散，太小则收敛很慢。 答题时还要结合课件语境：常考学习率过大/过小的训练现象，以及学习率调度的目的。
